...
def text_command_handler(message):
    ...
    if cmd in ["монетка", "coin", "монет"]:
        ...
        if not is_admin(user_id):
            bot_instance.bot.reply_to(message, "⚠️ Монетка и Монетка PvP сейчас отключены для обычных игроков.\nДоступно только администраторам (НЕ ДОРАБОТАНО!).")
            return
        if len(parts) > 1:
            sub = parts[1].lower()
...
def send_games_section(chat_id: int):
    ...
    text = (
        ...
        "🪙 <b>МОНЕТКА</b> (только для админов, НЕ ДОРАБОТАНО!)\n"
